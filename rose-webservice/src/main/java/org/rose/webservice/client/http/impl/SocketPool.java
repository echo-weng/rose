...
import org.rose.webservice.client.http.URLPoolStatus;
...
public final class SocketPool {
    ...
    private SocketWrapper createOrGetSocketWrapper() throws IOException {
        synchronized (lock) {
            if (idleSocketWrappers.size() > 0) {
                ...
                Socket socket = SocketUtils.createSocket(getSSLSocketFactory(), new URL(url),
    ...
    public synchronized void release(SocketWrapper socketWrapper) {
    ...
    private void closeSocket(SocketWrapper socketWrapper) {
        ...
        try {
            socket.close();
            if (Constant.LOG.isInfoEnabled()) {
                Constant.LOG.info("close socket [" + socketWrapper + "]");
            }
        } catch (Exception e) {
    ...
    public URLPoolStatus getPoolStatus() {
        URLPoolStatus URLPoolStatus = new URLPoolStatus();
        URLPoolStatus.setIdleCount(idleSocketWrappers.size());
        URLPoolStatus.setRunningCount(runningSocketCount);
        return URLPoolStatus;
    }
