...
import org.rose.webservice.client.http.impl.HttpSocketConnectionFactory;
import org.rose.webservice.client.https.AlwaysTrustSSLSocketFactory;

/**
 * classes such as ParameterClientImpl , SimpleClientImpl ,
 * SessionAwareParameterClient,WebServiceClient can extends this base class to
 * execute http request
 */
public abstract class BaseClient implements Client {

	// default content type "text/xml"
	private String contentType = CommonWebService.CONTENT_TYPE_TEXT_XML;
	// default encoding type "utf-8"
	private String encoding = CommonWebService.DEFAULT_ENCODING;
	...
	/**
	 * Get singleton HttpConnectionFactory instance . It will be used to
	 * generate the http connection instance could be (for
	 * examples:HttpSocketConnectionFactory
	 * /PoolableHttpSocketConnectionFactory/FixedURLConnectionFactory)
	 * 
	 * @return singleton HttpConnectionFactory instance
	 */
	public synchronized HttpConnectionFactory getHttpConnectionFactory() {
		if (null == httpConnectionFactory) {
			httpConnectionFactory = new HttpSocketConnectionFactory();
			httpConnectionFactory.setSSLSocketFactory(new AlwaysTrustSSLSocketFactory());
		}
	 ...
	 *            instance such as
	 *            HttpSocketConnectionFactory/PoolableHttpSocketConnectionFactory
	 *            /FixedURLConnectionFactory
	 */
	...
	/**
	 * all subclass base to use this execute method to send parameters to
	 * specific url
	 * 
	 * @param wrapper
	 *            url with alias
	 ...
	 *            http connection (request header , http method , content type ,
	 *            url encoding,parameters) ready
	 * @return response for object
	 */

	protected Object execute(URLWrapper wrapper, HttpConnectionContext httpConnectionContext,
		...
		return executor.execute(wrapper, httpConnectionContext, getHttpConnectionFactory(), httpConnectionTemplate);
	}
